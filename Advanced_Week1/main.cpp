...
int main()
{
    ...
    int i,eggs=3,hatchDays=5,alien[30],eggsLaid[30];
    ...
    for(i=1;i<30;i++)
    {
        ...
        if(i>(hatchDays-1))
        {
            alien[i]=alien[i-1]+eggsLaid[i-hatchDays];
            eggsLaid[i]=(alien[i]*eggs);
        }
    ...
    cout<<"Aliens 30 days after the invasion= "<<alien[29];
    return 0;
}
